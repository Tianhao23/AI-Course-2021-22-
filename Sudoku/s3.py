...
#myPuzzles = open("puzzles.txt").read().splitlines()
...
def update(change, dots, choice):
    return {i:(dots[i] -{choice}) if i in dots and i in nbrs[change] else dots[i] for i in dots} 
def bF(pzl, dots):
    ...
    for choice in dots.pop(change): #for every symbol you can use, add it to the first place needed to add a symbol
            ...
            #dot2 = {key:{*dots[key]}for key in dots}
            rem = set()
            for i in dots:
                if i in nbrs[change]:
                    rem.add(i)
                    dots[i] = dots[i]-{choice}
