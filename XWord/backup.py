...
def grabWord(trackletters, length):
    for i in range(len(bucketOfWords[length])):
        valid = True
        for k in trackletters:   
            if bucketOfWords[length][i][k].lower() !=trackletters[k].lower():
                valid = False
        if valid: 
            return bucketOfWords[length][i]
    return bucketOfWords[length][0]
...
def pWords(trackLetters, length):#list of possible words for a specific place on the xword
    ...
    for i in range(len(bucketOfWords[length])): #for every word of a certain length
        ...
        if valid: words.add(bucketOfWords[length][i])
    return words
...
def x2(pzl, usedWords): #update, instead of returning the puzzle, add it to a set
    ...
    for i in words:
        if i not in usedWords:
            subpzl = [*pzl]
            for idx in range(len(wordPos)): subpzl[wordPos[idx]] = i[idx]
            copyUsed = {*usedWords}
            copyUsed.add(i)
            pSol = x2(subpzl, copyUsed)
            if pSol: return pSol
    return ''
