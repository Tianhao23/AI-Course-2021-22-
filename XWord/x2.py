...
def createPossibles(wordIndeces, pzl):
    ...
    for word in wordIndeces: #for every possible place you can put a word
        ...
        possibles[tuple(word)] = posWords
    return possibles
def isInvalid(pos):
    for i in pos:
        if not pos[i] : return True
    return False
...
def minPos(pos, pzl):
    ...
    for i in pos:
        if (not wordPos and not words):
            if canFill(i, pzl):
                wordPos, words = i, pos[i]
        if len(pos[i]) < len(words) and canFill(i, pzl):
            wordPos, words = i,pos[i]
...
def update(p, changed, pzl): #updates the list of possible words
    ...
    for idx in changed:
        ...
        for cs in posCon[idx]:
            x =  wordIndeces[cs].index(idx)
            for i in p[tuple(wordIndeces[cs])]:
                if i[x] != letter:
                    if i in c[tuple(wordIndeces[cs])]:c[tuple(wordIndeces[cs])].remove(i)
    return c
