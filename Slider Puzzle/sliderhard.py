...
def makeBall(len, goal): #method to create the ball around goal
    parseMe = [(goal,0)]
    dctSeen = {goal: ""}
    for node, lvl in parseMe:
        if lvl> len: #if the radius is greater than the desired radius, return the ball
            return dctSeen   
        for nbr in neighbors(node): #gets neighbors of node
            if nbr not in dctSeen:
                path = createPath((nbr,node))
                moves = path + dctSeen[node]
                parseMe.append((nbr, lvl+1))
                dctSeen[nbr] = moves
def AStar(start, goal):
    global moves, ball
    shortest = "DDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDD"
    ...
    for e in openSet: #goes through each key in openSet
        for nde in openSet[e]: #if the key is not empty, it will go through each element in the value
            ...
            for i in psbeNbr[index]: #produces the neighbors of puzzle without using list
                ...
                if nbr in ball: #if path runs into ball
                    closedSet[nbr] = pzl
                    temp = nbr
                    output = []
                    ...
                    temp = ball[nbr] 
                    x = createPath(output[::-1]) + temp #compute the current path
                    if len(shortest) > len(x): #if this path is the shortest path, then update shortest path
                        shortest = x                    
                #if nbr == goal:
                #     closedSet[nbr] = pzl
                #     shortest = ball[nbr]
...
for sltn in range(len(myWords)): #searches for the solution for every puzzle
    ...
    if len(solve) > 0: #if a solution exists
        ...
        #     moves = solve
        print(str(sltn)+":",start, "solved in",c,"secs => path:", solve)
    else: #if solution does not exist
        print(str(sltn)+":", start, " is unsolvable => path:", solve)
