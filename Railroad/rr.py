...
# Tianhao Chen, pd.4import time, math, random
...
Root = Tk()
Root.title("Railroad Astar(Red is openSet, Blue is closedSet, Yellow is path)")
...
def drawLine(canvas, lat1, long1, lat2, long2, col): #draws the lines for the tkinter
   ...
   canvas.create_line(long1, 800-lat1, long2, 800-lat2, fill=col, width = 2)
...
def drawPath(path, canvas):
    for i in range(len(path)-2):
        stn1 = path[i]
        stn2 = path[i+1]
        lat1 = (stations[stn1][0]-9)/45 * 600
        long1 = (stations[stn1][1]+134)/70*830
        lat2 = (stations[stn2][0]-9)/45 * 600
        long2 = (stations[stn2][1]+134)/70*830
        drawLine(canvas, lat1, long1, lat2, long2, "Yellow")
        Root.update()
def createPath(path):
    output = []
    for city in path:
