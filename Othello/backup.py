...
def playGame(findBestMove, findMoves, makeMove, token):
    ...
    while True:
        moves, wtm =findMoves(brd, tknToPlay, 'XO'[tknToPlay=="X"])
        if not (moves):
        ...
        if tknToPlay != token: # if it's Random's turn:
            ...
            brd = makeMove(brd, tknToPlay, 'XO'[tknToPlay=="X"], int(transcript[-1]), wtm)
            #show2D(brd, tknToPlay, int(transcript[-1]), findMoves)
        else: # else it's Our turn
            transcript.append(findBestMove(moves, tknToPlay,'XO'[tknToPlay=="X"], brd, wtm))
            brd = makeMove(brd,  tknToPlay, 'XO'[tknToPlay=="X"], int(transcript[-1]), wtm)
            #show2D(brd, tknToPlay, int(transcript[-1]), findMoves)
