...
for m in move:
    ...
    if not pos or x not in pos:
        ...
        play = op
        op = temp
    board = makeMove(board.upper(), play, op, x)
    newPos = possible(board.upper(), op, play)
    if not newPos:
        newPos = possible(board.upper(), play, op)
        snapShot(board, play, newPos, m, play)
    else:
        snapShot(board, play, newPos, m, op)
        temp = play
        play = op
        op = temp
